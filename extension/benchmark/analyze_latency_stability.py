...
def analyze_latency_stability(primary_file, reference_file=None, output_dir="stability_analysis_results"):
    """
    ...
    for sheet, info in primary_datasets.items():
        generate_dataset_report(sheet, info['model'], info['device'], "Primary", info['df'], info['metrics'], output_dir)
    ...
    generate_summary_report(primary_datasets, reference_datasets if reference_file else None, output_dir)
...
def calculate_stability_metrics(df, raw_col, trimmed_col=None, tps_col=None):
    ...
    metrics['cv_raw_latency'] = (metrics['std_raw_latency'] / metrics['mean_raw_latency']) * 100
    metrics['iqr_raw_latency'] = np.percentile(raw_latency, 75) - np.percentile(raw_latency, 25)
    if trimmed_latency is not None:
        metrics['std_trimmed_latency'] = np.std(trimmed_latency, ddof=1)
        metrics['cv_trimmed_latency'] = (metrics['std_trimmed_latency'] / metrics['mean_trimmed_latency']) * 100
        metrics['iqr_trimmed_latency'] = np.percentile(trimmed_latency, 75) - np.percentile(trimmed_latency, 25)
    ...
    metrics['p99_p50_ratio_raw'] = metrics['p99_raw_latency'] / metrics['p50_raw_latency']
    
    if trimmed_latency is not None:
        if np.min(trimmed_latency) > 0:
            metrics['max_min_range_ratio_trimmed'] = np.max(trimmed_latency) / np.min(trimmed_latency)
        else:
            metrics['max_min_range_ratio_trimmed'] = float('inf')
            print(f"Warning: Minimum trimmed latency value is zero, max/min ratio set to infinity")
        
        metrics['p99_p50_ratio_trimmed'] = metrics['p99_trimmed_latency'] / metrics['p50_trimmed_latency']
    ...
    cv_score = max(0, 100 - (metrics['cv_raw_latency'] * 10))  # CV of 10% or more gets 0
    
    if metrics['max_min_range_ratio_raw'] == float('inf'):
        max_min_score = 0
    else:
        max_min_score = max(0, 100 - ((metrics['max_min_range_ratio_raw'] - 1) * 50))  # Ratio of 3.0 or more gets 0
    
    p99_p50_score = max(0, 100 - ((metrics['p99_p50_ratio_raw'] - 1) * 100))  # Ratio of 2.0 or more gets 0
    
    # Weighted average
    metrics['stability_score'] = (cv_weight * cv_score + 
...
def generate_dataset_report(sheet_name, model, device, dataset_type, df, metrics, output_dir):
    ...
    report_content.append(f"  - Median latency (P50): {metrics['median_raw_latency']:.2f} ms")
    if 'mean_trimmed_latency' in metrics and metrics['mean_trimmed_latency'] is not None:
        report_content.append(f"  - Mean trimmed latency: {metrics['mean_trimmed_latency']:.2f} ms")
        report_content.append(f"  - Median trimmed latency: {metrics['median_trimmed_latency']:.2f} ms")
    report_content.append("")
    
    # Dispersion metrics
    report_content.append("Dispersion Metrics:")
    report_content.append(f"  - Standard deviation: {metrics['std_raw_latency']:.2f} ms")
    report_content.append(f"  - Coefficient of variation (CV): {metrics['cv_raw_latency']:.2f}%")
    report_content.append(f"  - Interquartile range (IQR): {metrics['iqr_raw_latency']:.2f} ms")
    if 'std_trimmed_latency' in metrics and metrics['std_trimmed_latency'] is not None:
        report_content.append(f"  - Trimmed standard deviation: {metrics['std_trimmed_latency']:.2f} ms")
        report_content.append(f"  - Trimmed coefficient of variation: {metrics['cv_trimmed_latency']:.2f}%")
    report_content.append("")
    ...
    if metrics['max_min_range_ratio_raw'] == float('inf'):
        ...
        report_content.append(f"  - Max/Min ratio: {metrics['max_min_range_ratio_raw']:.4f}")
    report_content.append(f"  - P99/P50 ratio: {metrics['p99_p50_ratio_raw']:.4f}")
    if 'mean_rolling_std' in metrics:
        report_content.append(f"  - Mean rolling std (window=5): {metrics['mean_rolling_std']:.2f} ms")
    report_content.append("")
    
    if 'mean_trimming_effect_ratio' in metrics and metrics['mean_trimming_effect_ratio'] is not None:
        report_content.append("Intra-Jitter Metrics (variability within runs):")
        report_content.append(f"  - Mean trimming effect ratio: {metrics['mean_trimming_effect_ratio']*100:.2f}%")
        report_content.append(f"  - Max trimming effect ratio: {metrics['max_trimming_effect_ratio']*100:.2f}%")
        report_content.append("")
    ...
    if 'mean_tps' in metrics and metrics['mean_tps'] is not None:
        ...
        report_content.append(f"  - TPS coefficient of variation: {metrics['cv_tps']:.2f}%")
        report_content.append("")
    
    # Stability assessment
    report_content.append("Stability Assessment:")
    report_content.append(f"  - Overall stability score: {metrics['stability_score']:.1f}/100")
    report_content.append(f"  - Overall stability rating: {metrics['stability_rating']}")
    report_content.append("")
    ...
    if metrics['stability_rating'] == 'Excellent':
        report_content.append(f"  The benchmark shows excellent stability (score: {metrics['stability_score']:.1f}/100) with very low")
        report_content.append(f"  variation between runs (CV: {metrics['cv_raw_latency']:.2f}%).")
        report_content.append("  This indicates highly consistent performance suitable for latency-sensitive applications.")
    elif metrics['stability_rating'] == 'Good':
        report_content.append(f"  The benchmark shows good stability (score: {metrics['stability_score']:.1f}/100) with low")
        report_content.append(f"  variation between runs (CV: {metrics['cv_raw_latency']:.2f}%).")
        report_content.append("  Performance is consistent and predictable for most use cases.")
    elif metrics['stability_rating'] == 'Moderate':
        report_content.append(f"  The benchmark shows moderate stability (score: {metrics['stability_score']:.1f}/100) with noticeable")
        report_content.append(f"  variation between runs (CV: {metrics['cv_raw_latency']:.2f}%).")
        report_content.append("  While average performance is acceptable, occasional latency spikes may occur.")
    else:
        report_content.append(f"  The benchmark shows poor stability (score: {metrics['stability_score']:.1f}/100) with significant")
        report_content.append(f"  variation between runs (CV: {metrics['cv_raw_latency']:.2f}%).")
        report_content.append("  Performance is unpredictable and may lead to inconsistent user experience.")
    
    # Additional insights
    if 'mean_trimming_effect_ratio' in metrics and metrics['mean_trimming_effect_ratio'] is not None and metrics['mean_trimming_effect_ratio'] > 0.05:
        report_content.append("")
        report_content.append("  The significant difference between raw and trimmed means suggests")
        report_content.append(f"  considerable intra-run jitter ({metrics['mean_trimming_effect_ratio']*100:.1f}%) with occasional outliers within benchmark runs.")
    
    if metrics['max_min_range_ratio_raw'] != float('inf') and metrics['max_min_range_ratio_raw'] > 1.2:
        report_content.append("")
        report_content.append(f"  The max/min ratio of {metrics['max_min_range_ratio_raw']:.2f} indicates")
        report_content.append("  substantial performance differences between the best and worst runs.")
    
    if metrics['p99_p50_ratio_raw'] > 1.1:
        report_content.append("")
        report_content.append(f"  The P99/P50 ratio of {metrics['p99_p50_ratio_raw']:.2f} suggests")
        report_content.append("  occasional latency spikes that could affect tail latency sensitive applications.")
...
def generate_time_series_plot(dataset_name, df, output_dir, dataset_type):
    ...
    if 'trimmean_inference_latency(ms)' in df_sorted.columns:
        plt.plot(df_sorted['Date'], df_sorted['trimmean_inference_latency(ms)'], 'g-', label='Trimmed Latency')
    
    # Add rolling mean
    window = min(5, len(df_sorted))
    if window > 1:
        rolling_mean = df_sorted['InferenceTime'].rolling(window=window).mean()
        plt.plot(df_sorted['Date'], rolling_mean, 'r--', label=f'{window}-point Rolling Mean')
    ...
    print(f"Generated time series plot: {output_dir}/{dataset_name}_{dataset_type.lower()}_time_series.png")
    plt.close()

def generate_comparison_report(sheet_name, model, device, primary_metrics, reference_metrics, output_dir):
    ...
    headers = ["Metric", "Private (Primary)", "Public (Reference)", "Difference", "% Change"]
    rows = []
    ...
    if primary_metrics['stability_score'] > reference_metrics['stability_score']:
        if reference_metrics['stability_score'] != 0:
            diff_pct = ((primary_metrics['stability_score'] - reference_metrics['stability_score']) / 
                        reference_metrics['stability_score'] * 100)
            report_content.append(f"  Private environment shows better stability with a {diff_pct:.1f}% higher stability score.")
        else:
            report_content.append(f"  Private environment shows better stability.")
        report_content.append(f"  (Private: {primary_metrics['stability_score']:.1f}/100 vs Public: {reference_metrics['stability_score']:.1f}/100)")
    elif primary_metrics['stability_score'] < reference_metrics['stability_score']:
        if primary_metrics['stability_score'] != 0:
            diff_pct = ((reference_metrics['stability_score'] - primary_metrics['stability_score']) / 
                        reference_metrics['stability_score'] * 100)
            report_content.append(f"  Public environment shows better stability with a {diff_pct:.1f}% higher stability score.")
        else:
    ...
    if primary_metrics['cv_raw_latency'] < reference_metrics['cv_raw_latency']:
        if reference_metrics['cv_raw_latency'] != 0:
            diff_pct = ((reference_metrics['cv_raw_latency'] - primary_metrics['cv_raw_latency']) / 
                        reference_metrics['cv_raw_latency'] * 100)
            report_content.append(f"  Private environment has {diff_pct:.1f}% lower coefficient of variation, indicating more consistent performance.")
        else:
            report_content.append(f"  Private environment has lower coefficient of variation, indicating more consistent performance.")
    elif primary_metrics['cv_raw_latency'] > reference_metrics['cv_raw_latency']:
        if reference_metrics['cv_raw_latency'] != 0:
            diff_pct = ((primary_metrics['cv_raw_latency'] - reference_metrics['cv_raw_latency']) / 
                        reference_metrics['cv_raw_latency'] * 100)
            report_content.append(f"  Public environment has {diff_pct:.1f}% lower coefficient of variation, indicating more consistent performance.")
        else:
            report_content.append(f"  Public environment has lower coefficient of variation, indicating more consistent performance.")
    
    # Compare latency
    if primary_metrics['mean_raw_latency'] < reference_metrics['mean_raw_latency']:
        if reference_metrics['mean_raw_latency'] != 0:
            diff_pct = ((reference_metrics['mean_raw_latency'] - primary_metrics['mean_raw_latency']) / 
                        reference_metrics['mean_raw_latency'] * 100)
            report_content.append(f"  Private environment has {diff_pct:.1f}% lower mean latency, indicating better performance.")
        else:
            report_content.append(f"  Private environment has lower mean latency, indicating better performance.")
    elif primary_metrics['mean_raw_latency'] > reference_metrics['mean_raw_latency']:
        if primary_metrics['mean_raw_latency'] != 0:
            diff_pct = ((primary_metrics['mean_raw_latency'] - reference_metrics['mean_raw_latency']) / 
                        reference_metrics['mean_raw_latency'] * 100)
            report_content.append(f"  Public environment has {diff_pct:.1f}% lower mean latency, indicating better performance.")
        else:
            report_content.append(f"  Public environment has lower mean latency, indicating better performance.")
    
    # Recommendation
    report_content.append("")
    report_content.append("Recommendation:")
    if primary_metrics['stability_score'] > reference_metrics['stability_score']:
        report_content.append("  The private environment provides better stability for this model+device combination.")
        report_content.append("  It is recommended for applications where consistent performance is critical.")
    elif primary_metrics['stability_score'] < reference_metrics['stability_score']:
        report_content.append("  The public environment provides better stability for this model+device combination.")
        report_content.append("  Consider investigating factors affecting stability in the private environment.")
    else:
        report_content.append("  Both environments provide similar stability. Other factors like cost or availability")
...
def generate_intra_primary_summary(primary_datasets, output_dir):
    ...
    report_content.append(tabulate(df, headers='keys', tablefmt='grid', floatfmt='.2f', showindex=False))
    report_content.append("")
    ...
    report_content.append(f"  Best stability: {best_dataset['Sheet']} (Score: {best_dataset['Stability Score']:.1f}/100)")
    report_content.append(f"  Worst stability: {worst_dataset['Sheet']} (Score: {worst_dataset['Stability Score']:.1f}/100)")
    report_content.append("")
    ...
    if len(models) > 1:
        ...
        model_stats = model_stats.sort_values(('Stability Score', 'mean'), ascending=False)
        
        report_content.append(tabulate(model_stats, headers='keys', tablefmt='grid', floatfmt='.2f'))
        
        best_model = model_stats['Stability Score']['mean'].idxmax()
        report_content.append(f"  Most stable model: {best_model} (Avg. Score: {model_stats.loc[best_model, ('Stability Score', 'mean')]:.1f}/100)")
        report_content.append("")
    ...
    if len(devices) > 1:
        ...
        # Sort by mean stability score (descending)
        device_stats = device_stats.sort_values(('Stability Score', 'mean'), ascending=False)
        
        report_content.append(tabulate(device_stats, headers='keys', tablefmt='grid', floatfmt='.2f'))
        
        best_device = device_stats['Stability Score']['mean'].idxmax()
        report_content.append(f"  Most stable device: {best_device} (Avg. Score: {device_stats.loc[best_device, ('Stability Score', 'mean')]:.1f}/100)")
        report_content.append("")
    ...
    if len(models) > 1:
        ...
        report_content.append(f"  - {most_stable_model} shows the most consistent performance across devices.")
        report_content.append(f"  - {least_stable_model} shows more variability and may need further optimization.")
    
    if len(devices) > 1:
        device_cv = df.groupby('Device')['CV (%)'].mean()
        most_stable_device = device_cv.idxmin()
        least_stable_device = device_cv.idxmax()
        report_content.append(f"  - {most_stable_device} provides the most stable environment for model execution.")
        report_content.append(f"  - {least_stable_device} shows higher variability and may not be ideal for latency-sensitive applications.")
    
    # General recommendations
    report_content.append("  - For critical applications requiring consistent performance, prefer:")
    report_content.append(f"    * Model: {best_model if len(models) > 1 else df['Model'].iloc[0]}")
    report_content.append(f"    * Device: {best_device if len(devices) > 1 else df['Device'].iloc[0]}")
...
def generate_summary_report(primary_datasets, reference_datasets, output_dir):
    ...
    primary_df = pd.DataFrame(primary_data).sort_values('Stability Score', ascending=False)
    
    report_content.append("Primary (Private) Datasets Summary:")
    report_content.append(tabulate(primary_df, headers='keys', tablefmt='grid', floatfmt='.2f', showindex=False))
    report_content.append("")
    ...
    if reference_datasets:
        ...
        reference_df = pd.DataFrame(reference_data).sort_values('Stability Score', ascending=False)
        
        report_content.append("Reference (Public) Datasets Summary:")
        report_content.append(tabulate(reference_df, headers='keys', tablefmt='grid', floatfmt='.2f', showindex=False))
        report_content.append("")
            ...
            for ref_sheet in reference_datasets:
                ...
                if normalized_primary == normalized_ref:
                    ...
                    comparison_data.append({
                        ...
                        'Score Diff': primary_metrics['stability_score'] - reference_metrics['stability_score'],
                        'Private CV (%)': primary_metrics['cv_raw_latency'],
                        'Public CV (%)': reference_metrics['cv_raw_latency'],
                        'CV Diff (%)': primary_metrics['cv_raw_latency'] - reference_metrics['cv_raw_latency']
                    })
        
        if comparison_data:
            comparison_df = pd.DataFrame(comparison_data).sort_values('Score Diff', ascending=False)
            
            report_content.append("Private vs Public Comparison:")
            report_content.append(tabulate(comparison_df, headers='keys', tablefmt='grid', floatfmt='.2f', showindex=False))
            report_content.append("")
            
            # Count datasets where private is better
            private_better_count = sum(1 for row in comparison_data if row['Score Diff'] > 0)
            public_better_count = sum(1 for row in comparison_data if row['Score Diff'] < 0)
            equal_count = sum(1 for row in comparison_data if row['Score Diff'] == 0)
            
            report_content.append(f"Private environment is more stable in {private_better_count} of {len(comparison_data)} cases.")
            report_content.append(f"Public environment is more stable in {public_better_count} of {len(comparison_data)} cases.")
            if equal_count > 0:
                report_content.append(f"Both environments show equal stability in {equal_count} of {len(comparison_data)} cases.")
            report_content.append("")
    ...
    report_content.append(f"  - Most stable configuration: {best_primary['Dataset']} (Score: {best_primary['Stability Score']:.1f}/100)")
    report_content.append(f"    Model: {best_primary['Model']}, Device: {best_primary['Device']}")
    
    # General recommendations
    report_content.append("")
    report_content.append("General Recommendations:")
    report_content.append("  1. For datasets with 'Poor' or 'Moderate' stability, investigate potential causes")
    report_content.append("     such as thermal throttling, background processes, or power management settings.")
    report_content.append("  2. Consider increasing warm-up iterations for datasets with high CV values.")
    report_content.append("  3. For critical applications, prefer models and devices with 'Good' or 'Excellent' stability.")
    if reference_datasets and comparison_data:
        if private_better_count > public_better_count:
            report_content.append("  4. Private environments generally provide better stability and should be preferred")
            report_content.append("     for production deployments where consistent performance is critical.")
        elif public_better_count > private_better_count:
            report_content.append("  4. Public environments show better stability in most cases. Consider investigating")
            report_content.append("     factors affecting stability in the private environment.")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analyze ML model latency stability from benchmark data.')
    parser.add_argument('primary_file', help='Path to Excel file containing primary (private) benchmark data')
    parser.add_argument('--reference_file', help='Path to Excel file containing reference (public) benchmark data for comparison', default=None)
    parser.add_argument('--output-dir', default='stability_analysis_results', 
